...
//				this.MinesAndMinesweepers(ShipType.MINE50);
//			else if (!capitulated && input.equals("7"))
//				this.MinesAndMinesweepers(ShipType.MINESWEEPER);
...
//		return false;
	...
	private void fightSimulation()
	{
		...
		while (true)
		{
			int offenderCount = this.fightSimulationGetBattleships(VegaResources.FightSimulationAttackerCount(true));
			if (offenderCount < 0)
			{
				this.game.getConsole().clear();
				return;
			...
			int offenderBonus = this.fightSimulationGetBonus();
			if (offenderBonus < 0)
			{
				this.game.getConsole().clear();
				return;
			...
			int defenderCount = this.fightSimulationGetBattleships(VegaResources.FightSimulationPlanetCount(true));
			if (defenderCount < 0)
			{
				this.game.getConsole().clear();
				return;
			...
			int defenderBonus = this.fightSimulationGetBonus();
			if (defenderBonus < 0)
			{
				this.game.getConsole().clear();
				return;
	...
	private int fightSimulationGetBattleships(String text)
	{
		while(true)
		{
			...
			try
			{
				count = Math.abs(Integer.parseInt(input.getInputText()));
			}
			catch (Exception x)
			{
				this.game.getConsole().outInvalidInput();
	...
	private int fightSimulationGetBonus()
	{
		...
		for (int i = 0; i <= Planet.MAX_BONUS; i++)
		{
			allowedKeys.add(
					new ConsoleKey(
							Integer.toString(i),
							CommonUtils.round(Evaluation.getCombatStrength(i) * 100) + "%"));
		}
		...
		while(true)
		{
			...
			try
			{
				bonus = Math.abs(Integer.parseInt(input.getInputText()));
			}
			catch (Exception x)
			{
				this.game.getConsole().outInvalidInput();
			...
			if (bonus >= 0 && bonus <= Planet.MAX_BONUS) return bonus;
			this.game.getConsole().outInvalidInput();
		}
	}
...
//		this.game.getConsole().clear();
	...
	private void MinesAndMinesweepers(ShipType shipCategory)
	{
		...
		boolean transfer = transferOptional.get();
		...
		do
		{
			inputDestination = this.game.getPlanetInput(
					transfer ?
							...
							VegaResources.DestinationSector(true), 
					transfer,
					!transfer);
	...
	private void Patrols()
	{
		...
		this.game.getPlanets()[planetIndexStart].decrementShipsCount(ShipType.PATROL, 1);
	...
	private void spiesTransports(ShipType type)
	{
		...
		do
		{
			...
			if (this.game.getPlanets()[planetIndexStart].getShipsCount(type) < 1)
			{
				this.game.getConsole().appendText(VegaResources.NoShipOfSelectedTypeAvailable(true));
				this.game.getConsole().lineBreak();
				continue;
	...
	private void updateBoardEnterMoves(int playerIndex, Ship startingShip)
	{
		// Planets
		...
		for (int planetIndex = 0; planetIndex < this.game.getPlanetsCount(); planetIndex++)
		{
			...
			// Draw darker if details are not visible
			scbPlanets.add(new ScreenContentBoardPlanet(
								...
								planet.areDetailsVisibleForPlayer(playerIndex) ?
										...
										Colors.getColorIndexDarker(planet.getOwnerColorIndex(this.game)),
								null));
		}
		
		// Ships (there are only ships in the loop that the player is allowed to see)
		...
		for (Ship ship: this.game.getShips())
		{
			...
			{
				scbObjects.add(this.game.updateBoardGetBlackHoleScreenContentBoardObject(ship));
				continue;
			}
			
			byte color = ship.getOwnerColorIndex(this.game);
			if (startingShip != null) color = Colors.getColorIndexDarker(color);
		...
		this.game.setScreenContentBoard(
